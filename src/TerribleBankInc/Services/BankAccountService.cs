...
public class BankAccountService : IBankAccountService
{
    ...
    {
        return (
            await _bankAccountRepository.Get(x => x.ID == id, $"{nameof(BankAccount.Client)}")
        ).FirstOrDefault();
    }
    ...
    {
        return await _bankAccountRepository.Get(
            x => x.Enabled == false && x.Approved.HasValue && x.Approved.Value,
            $"{nameof(BankAccount.Client)}"
        );
    }
    ...
    {
        return await _bankAccountRepository.Get(
            x => x.Enabled == false && !x.Approved.HasValue,
            $"{nameof(BankAccount.Client)}"
        );
    }
    ...
    {
        BankAccount account = _mapper.Map<BankAccount>(newAccount);
    ...
    {
        BankAccount account = await GetById(id);
