...
public class BankAccountController : BaseController
{
    ...
    public async Task<IActionResult> Index()
    {
        ...
        ClientBankAccountsViewModel vm = new ClientBankAccountsViewModel
        {
            ActiveAccounts = accounts
                .Where(x => x.Approved.HasValue && x.Approved.Value && x.Enabled)
                .OrderBy(x => x.Currency)
                .ToList(),
            DisabledAccounts = accounts
                .Where(x => !x.Enabled && x.Approved.HasValue && x.Approved.Value)
                .OrderBy(x => x.Currency)
                .ToList(),
            PendingAccounts = accounts
                .Where(x => !x.Approved.HasValue && !x.Enabled)
                .OrderBy(x => x.Currency)
                .ToList(),
            RejectedAccounts = accounts
                .Where(x => x.Approved.HasValue && x.Approved.Value == false)
                .OrderBy(x => x.Currency)
                .ToList()
        };
        return View(vm);
    ...
    {
        BankAccount account = await _bankAccountService.GetById(id);
        if (account == null)
            return NotFound();
        return View(account);
    }
    ...
    public async Task<IActionResult> RequestNew(
        NewBankAccountRequestViewModel newBankAccountRequest
    )
    {
        if (ModelState.IsValid)
        {
            bool result = await _bankAccountService.RequestNewBankAccount(newBankAccountRequest);
    ...
    {
        bool result = await _bankAccountService.BlockAccount(ID);
        return RedirectToAction(nameof(Index));
    }
}
