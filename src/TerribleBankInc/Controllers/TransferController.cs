...
public class TransferController : BaseController
{
    ...
    public async Task<IActionResult> CreateTransfer()
    {
        ...
        BankTransactionViewModel vm =
            new() { ActiveAccounts = new List<SelectListItem>(), SourceClientId = 1 };
    ...
    public async Task<IActionResult> CreateTransfer(BankTransactionViewModel transfer)
    {
        ...
        if (ModelState.IsValid)
        {
            ...
            bool result = await _bankTransferService.Send(transfer);
            if (result)
        ...
        var activeAccounts = (
            await _bankAccountService.GetAllAccountsForClient(transfer.SourceClientId)
        )
            .Where(x => x.Enabled && x.Approved.HasValue && x.Approved.Value)
            .ToList();
    ...
    private void PopulateSelectListWithAccounts(
        ...
        foreach (BankAccount account in accounts)
            selectList.Add(
                new SelectListItem
                {
                    ...
                    Text =
                        $"{account.AccountNumber} | {Enum.GetName(typeof(CurrencyTypes), account.Currency)} | Balance: {account.Balance}"
                }
            );
    }
}
